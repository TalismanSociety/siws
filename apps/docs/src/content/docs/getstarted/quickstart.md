...
The message includes a **nonce** — just a random single-use token (e.g. `crypto.randomUUID()`) generated by your backend, so a captured signature can't be replayed. Have an endpoint return one, storing it in an httpOnly cookie: the browser sends it back automatically at verification time, no database needed.
...
// prevent replay attacks: the signed nonce must be the one you issued,
